...
__export(index_exports, {
  ...
  createWorkerBridge: () => createWorkerBridge,
...
function useComputed(fn) {
  const node = (0, import_react.useMemo)(() => new ComputedNode(fn), []);
  const [, forceUpdate] = (0, import_react.useState)(0);
  (0, import_react.useEffect)(() => {
    const effect = new EffectNode(() => {
      node.get();
...
function useObserver(render) {
  const [, forceUpdate] = (0, import_react.useState)(0);
  const effectRef = (0, import_react.useRef)(null);
